...
(function () {
  ...
  let current = clamp(parseFloat(localStorage.getItem('zoomScale') || range.value || '1') || 1);
  ...
  // Range interactions
  range.addEventListener('pointerdown', () => { userSliding = true; });
  range.addEventListener('pointerup',   () => { userSliding = false; });
  range.addEventListener('change',      () => { userSliding = false; });
